...
public:
  ...
  const std::vector<std::vector<uint16_t> >& getDistances() const {
    return distances;
  }
  void setDistances(const std::vector<std::vector<unsigned short> >& distances) {
  ...
  uint16_t getMinCapacity() const {
    return criteria.minCapacity();
  }
  uint16_t getMaxDistance() const {
    return criteria.maxDistance();
  }
  uint16_t getMinDistance() const {
    return criteria.minDistance();
  }
